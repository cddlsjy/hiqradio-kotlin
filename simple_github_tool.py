...
class App:
    def __init__(self, root):
        ...
        self.work_dir = self.config.get("work_dir", "")       # 工作目录
    ...
    def on_close(self):
        self.config.update({
            ...
            "download_dir": self.dl_dir_var.get() if hasattr(self, 'dl_dir_var') else "",  # 保留旧配置兼容
            "auto_extract": self.auto_extract,
            "delete_zip_after_extract": self.delete_zip_after_extract,
            "auto_upload": self.auto_upload,
            "upload_target": self.upload_target,
